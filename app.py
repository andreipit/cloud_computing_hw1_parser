...
def df2string(df):
    ...
    for index, row in df.iterrows():
        result = result + '----------' + '\n'
        for i in range(1,4):
            result2 = str(df.columns[i] + '=' + str(row[df.columns[i]]))
...
def hello():
    ...
    # return 'Visited ulrs contain this requests:\n'.format(df2string(df_str))
    return df2string(df_str)
    # return html_table(df_str)
    # return 'Hello World!'
